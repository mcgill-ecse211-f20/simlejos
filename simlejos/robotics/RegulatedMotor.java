...
/**
 * Wrapper class around Webots' Motor class to simulate Lejos' RegulatedMotor class
 * The main difference is that the constructor uses the name of the motor object on the robot rather than the port number
 Lejos API:
  http://www.lejos.org/ev3/docs/lejos/hardware/motor/EV3LargeRegulatedMotor.html
  http://www.lejos.org/ev3/docs/lejos/robotics/RegulatedMotor.html
 Webots API:
  https://cyberbotics.com/doc/reference/motor?tab-language=java
  https://cyberbotics.com/doc/reference/positionsensor?tab-language=java
...
public class RegulatedMotor {
  ...
  /**
   * The Webots motor we will interface with
   */
  private Motor target;

  /**
   * The Webots positionSensor we will interface with
   */
  ...
  /**
   * Create a new RegulatedMotor
   * @param port
   * Reference to the port where the motor is connected
   */
  ...
  public boolean isMoving() {
   return Math.abs(speed)>0.0001?true:false;
  }
  ...
  public void waitUntilTargetReached(){
    ...
    try {
      ...
      if(target.getVelocity()==0)return;
    } catch (Exception e) {
      System.err.println("EV3LargeRegulatedMotor waitUntilTargetReached exception: " + e.getMessage());
    } finally {
      motorLock.unlock();
    }
    //Wait while motor is not at the target position
    while(Math.abs(getSensorValue()-target.getTargetPosition())>0.01){
      //System.out.println("target:" + target.getTargetPosition() + "\tposition: " + getSensorValue());
  ...
  /**
   * Read sensor value, relative to last tacho reset
   */
   ...
   try {
    ...
    if(Double.isNaN(value)){
     value=0;
      ...
      System.err.println("EV3LargeRegulatedMotor getOffsetSensorValue exception: " + e.getMessage());
   } finally {
    sensorLock.unlock();
   }
   return value;
